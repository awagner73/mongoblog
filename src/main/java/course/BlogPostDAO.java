...
public class BlogPostDAO {
	...
	// changes to templates and post retrieval code.
    	...
    	System.out.println(">>>>>>>>>>>>>>>>>>");
    	BasicDBObject basicDBObject = new BasicDBObject("permalink", permalink);
    	BasicDBObject upd = new BasicDBObject("$inc",new BasicDBObject("comments."+ordinal+".num_likes",1));
    	//DBObject post = postsCollection.findOne(basicDBObject);
    	WriteResult update = postsCollection.update(basicDBObject, upd);
    	
    	
    	System.out.println("["+update.getN() + " -" + update.getError() +"]");
    	System.out.println(">>>>>>>>>>>>>>>>>>");
